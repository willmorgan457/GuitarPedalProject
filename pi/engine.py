...
def callback(indata, outdata, frames, time, status):
    ...
    if neural_on:
        audio_1d = guitar[:, 0].flatten()
        processed_1d = neural_amp.process(audio_1d)
        audio = processed_1d[np.newaxis, :]
    else:
        audio = guitar.T
...
def input_loop():
    global board, reverb_on, delay_on, neural_on
    while True:
        key = input()
        if key == '1':
...
def run_engine():
    ...
    with sd.Stream(
        ...
        print("Audio engine running. Press 1=reverb, 2=delay, 3=neural amp.")
        t.join()
